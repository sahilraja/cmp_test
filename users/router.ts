...
router.post(`/:id/replace`, async (req, res, next) => {
    try {
        res.status(OK).send(await replaceUser(res.locals.user._id, req.body.replaceTo, (req as any).token))
    } catch (error) {
        next(error)
    }
})
