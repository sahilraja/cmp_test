...
async function sendAllNotificationsOnFinancialInfo(members:any, activityBy: any, projectName:string) {
  ...
  const userRolesInfo:any = await Promise.all(fetchedUsers.map(async(user: any) => await userRoleAndScope(user._id)))
  let userRoles:any = userRolesInfo.map((role: any) => (role.data || [""])[0])
  fetchedUsers = fetchedUsers.map((user: any, index: number) => ({...user, role:userRoles[index]}))
  const filterNonCapableUsers = await Promise.all(fetchedUsers.map(async(user: any) =>await checkRoleScope(user.role, `manage-project-utilized-fund`)))
